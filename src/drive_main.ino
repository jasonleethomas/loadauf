...
int const servo_full_speeds[sensor_count] = { 35, 145 };
int const servo_base_speeds[sensor_count] = { 70, 110 };
int const servo_stop_speed = 90; 

// to-do: tune front (11/27/15)
float const pid_proportional[sensor_count] = { 0, 0, 0.005 };
float const pid_integral[sensor_count] = { 0, 0, 0 };
float const pid_derivative[sensor_count] = { 0, 0, 0 };

long const distance_boundary[sensor_count] = { 0, 0, 5 }; // meters
...
long sensor_read_distance(int echo_pin, int trigger_pin)
{
  ...
  return (echo_duration / 29 / 2) * 100; // meters
}
